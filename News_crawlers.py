...
driver.get("https://udn.com/search/word/2/%E8%B3%87%E8%A8%8A%E5%AE%89%E5%85%A8")

#步驟3 將搜尋結果展開至我們要的大小
date = "2019-10-04"
#i = 0 #滾輪下拉次數
time.sleep(10) #等待網頁開好
driver.execute_script("window.scrollBy(0,3200)") #經過測試，第一次移動3200是最剛好可以讓網頁繼續讀取的距離
time.sleep(10) #下拉第一次後，保留時間讓網頁跑出動態結果
while driver.page_source.find(date) == -1 : #持續下拉網頁，直到翻到指定日期的新聞
    #print("進入")
    driver.execute_script("window.scrollBy(0,4000)") #經過測試，每次移動4000是最剛好可以讓網頁繼續讀取的距離
